...
class Simulator_rpy(Node):
    ...
    def joy_callback(self, joy: Joy):
        ...
        # posAndOri = p.getBasePositionAndOrientation(self.robot)
        # ori = p.getEulerFromQuaternion(posAndOri[1])
    ...
    def timer_callback(self):
        ...
        if self.selfBalance_flag==False:
            ...
            if self.recover_flag == True:
                ...
                self.roll_angle += self.roll / 2
                self.pitch_angle += self.pitch / 2
                self.yaw_angle += self.yaw / 2
...
##   self-balancing model
            ...
            if np.abs(self.ori2[0]+self.ori2[1]) *180/np.pi > 2  :
                self.get_logger().info(f'PD controller working')
...
def main():
    ...
    table = p.loadURDF("src/urdf1/hexapod_v2.urdf", [10, 0, 1], useMaximalCoordinates=False,
